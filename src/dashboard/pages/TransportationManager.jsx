...
import { Bus, Plus, Trash2, Edit, X, Users, ShieldCheck } from 'lucide-react';
...
const TransportationManager = () => {
  ...
  const handleOpenAdd = () => {
    ...
    setForm({
      ...
      features: ['تكييف كامل', 'واي فاي مجاني', 'سائق محترف', 'شاشات عرض', 'مقاعد جلدية']
    });
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#161b22] border border-slate-800 rounded-3xl p-6">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-3">
            ...
            <span>إدارة أسطول التنقل والسيارات (Transportation Fleet)</span>
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {transportation.map((item) => (
          <div 
            key={item.id}
            className="bg-[#161b22] border border-slate-800 rounded-3xl overflow-hidden hover:border-purple-500/30 transition-all flex flex-col justify-between"
          >
            <div>
              <div className="h-44 bg-slate-800 relative">
                <img 
                  src={item.image || '/imgs/transportation/bus1.jpeg'} 
                  alt={item.name}
            ...
            <div className="p-4 border-t border-slate-800/80 flex items-center justify-between bg-slate-900/40">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setDeleteConfirmId(item.id)} className="p-2 rounded-xl bg-slate-800 text-red-400 hover:text-white"><Trash2 size={16} /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#161b22] border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4">
            <h3 className="font-bold text-lg text-white">تأكيد حذف المركبة</h3>
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => { deleteTransport(deleteConfirmId); setDeleteConfirmId(null); }} className="px-4 py-2 rounded-xl bg-red-500 text-white text-xs font-bold">حذف</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#161b22] border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 mb-1">اسم المركبة / الحافلة</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="w-full bg-[#0d1117] border border-slate-800 rounded-xl p-2.5 text-white">
                    ...
                    <option value="private">سيارة خاصة (Private Car)</option>
                  </select>
                </div>
                <div>
