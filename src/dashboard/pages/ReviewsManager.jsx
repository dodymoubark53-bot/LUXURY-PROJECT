    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#161b22] border border-slate-800 rounded-3xl p-6">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-3">
            ...
            <span>إدارة التعليقات والتقييمات على الرحلات (Reviews & Comments)</span>
          </h1>
          <p className="text-slate-400 text-xs mt-1">
            مراجعة وإضافة وتعديل آراء وتجربة العملاء على الرحلات السياحية المختلفة.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {reviews.map((rev) => (
          <div 
            key={rev.id} 
            className="bg-[#161b22] border border-slate-800 rounded-3xl p-6 flex flex-col justify-between space-y-4 hover:border-rose-500/30 transition-all"
          >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#161b22] border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 mb-1">الرحلة المستهدفة</label>
                <select 
                  value={form.tripId} 
                  onChange={e => {
              ...
              <div>
                <label className="block text-slate-300 mb-1">نص التعليق والملاحظات</label>
