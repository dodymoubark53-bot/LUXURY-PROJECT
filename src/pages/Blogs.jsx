    ...
    <div className="w-full bg-[#F8F9FF] pb-24">
      ...
      <section className="relative h-[40vh] flex items-center justify-center overflow-hidden">
        ...
        <motion.div className="relative z-10 text-center" variants={staggerContainer} initial="hidden" animate="visible">
          <motion.h1 variants={fadeInUp} className="text-display-xl text-white">{t('blogs.title', 'Travel Journal')}</motion.h1>
        </motion.div>
      </section>
        ...
        >
          {blogs.map(blog => (
            <motion.article key={blog.id} variants={fadeInUp}>
              <motion.div className="bg-white rounded-2xl overflow-hidden shadow-card group h-full flex flex-col" variants={cardHover} initial="rest" whileHover="hover" animate="rest">
                <Link to={`/blogs/${blog.slug}`} className="relative block" style={{ height: '220px', borderRadius: '12px 12px 0 0', overflow: 'hidden' }}>
                  <div className="absolute top-4 left-4 z-10 bg-gold-500 text-[#1A1A2E] text-caption uppercase px-3 py-1 rounded-full shadow-md">{t(`blogs.cat.${blog.category}`, blog.category)}</div>
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <div className="flex justify-between items-center text-caption text-[#8A8AAE] mb-3">
                    <span>{t(`blogs.${blog.date}`, blog.date)}</span>
                    <span>{t(`blogs.${blog.readTime}`, blog.readTime)}</span>
                  </div>
                  <Link to={`/blogs/${blog.slug}`}>
                    <h3 className="text-display-md text-[#1A1A2E] mb-3 text-xl line-clamp-2 hover:text-gold-500 transition-colors cursor-pointer">{t(`blogs.${blog.title}`, blog.title)}</h3>
                  </Link>
