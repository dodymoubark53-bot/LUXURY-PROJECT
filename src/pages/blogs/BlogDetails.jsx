...
const BlogDetails = () => {
  ...
  if (!blog) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-obsidian-50">
        <h1 className="text-display-lg text-obsidian-900">Article not found</h1>
      </div>
    );
  }
  ...
  return (
    <div className="w-full bg-obsidian-50 pb-24">
      <Helmet>
        <title>{blog.title} | Luxury Travel Journal</title>
      ...
      <section className="relative h-[60vh] flex items-end justify-center overflow-hidden pb-16">
        <div className="absolute inset-0 z-0">
          <img
            src={`${blog.img}?w=1600&q=90&fit=crop`}
            alt={blog.title}
        ...
        >
          ...
          <motion.h1 variants={fadeInUp} className="text-display-lg md:text-display-xl text-ivory-50 mb-6">
            {blog.title}
          </motion.h1>
          <motion.p variants={fadeInUp} className="text-body-md text-ivory-300">
            By <span className="text-ivory-50 font-medium">{blog.author}</span> &bull; {blog.readTime}
          </motion.p>
        </motion.div>
      </section>
      ...
      <section className="container mx-auto px-6 mt-16 mb-24 max-w-3xl">
        ...
        <div className="mt-12 pt-8 border-t border-obsidian-900/10 flex justify-between items-center">
          <div className="flex gap-2">
            <span className="px-4 py-2 bg-ivory-50 rounded-full text-caption text-obsidian-500 shadow-sm border border-obsidian-900/5 hover:border-gold-300 transition-colors cursor-pointer">#{blog.category.replace(/\s+/g, '')}</span>
          ...
          <button className="text-gold-500 hover:text-gold-700 font-medium text-body-sm transition-colors">Share Article</button>
        </div>
      </section>
      ...
      <section className="bg-ivory-50 py-24 border-t border-obsidian-900/5">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-display-lg text-obsidian-900">Related Articles</h2>
              ...
              <motion.article key={relBlog.id} variants={fadeInUp}>
                <div className="bg-obsidian-50 rounded-2xl overflow-hidden group h-full flex flex-col border border-obsidian-900/5 transition-all duration-300 hover:shadow-[0_16px_48px_rgba(10,8,4,0.08)]">
                  <Link to={`/blogs/${relBlog.slug}`} className="relative h-56 overflow-hidden block">
                    <div className="absolute top-4 left-4 z-10 bg-gold-500 text-obsidian-900 text-caption uppercase px-3 py-1 rounded-full shadow-md">{relBlog.category}</div>
                    <img src={`${relBlog.img}?auto=compress&cs=tinysrgb&w=800&fit=crop&crop=center`} alt={relBlog.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
                  </Link>
                  <div className="p-8 flex flex-col flex-grow">
                    ...
                    <Link to={`/blogs/${relBlog.slug}`}>
                      <h3 className="text-display-md text-obsidian-900 mb-3 text-lg line-clamp-2 hover:text-gold-500 transition-colors cursor-pointer">{relBlog.title}</h3>
                    </Link>
